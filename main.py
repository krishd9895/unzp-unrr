...
def handle_document(message):
    try:
        ...
        for item in os.listdir():
            if item.endswith('.zip') or item.endswith('.rar') or item.startswith('extracted_files_'):
        ...
        if file_name.endswith('.zip'):
            ...
            bot.reply_to(message, "Unsupported file format. Please send a .zip or .rar file.")
            return
        ...
        for subdir, _, _ in os.walk(destination_dir):
            if subdir != destination_dir:
                bot.send_message(message.chat.id, f"Files in {os.path.relpath(subdir, destination_dir)}:")
